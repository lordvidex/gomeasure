...

{
  ...
  curl -s --compressed "https://${GITHUB_USERNAME}.github.io/${GITHUB_REPOSITORY}/KEY.gpg" | apt-key add -

  # Curls the path to `.deb` file and cat it to the apt `source.list`
  curl -s --compressed -o /etc/apt/sources.list.d/gomeasure.list "https://${GITHUB_USERNAME}.github.io/${GITHUB_REPOSITORY}/gomeasure.list"
